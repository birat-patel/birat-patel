# 👋 Hi, I'm Birat Kumar 

💻 Frontend Developer | 🌐 Web Designer | 📍 From Rohtas, Bihar  

I’m a self-taught Frontend Developer and BCA graduate who enjoys building clean, responsive, and user-friendly websites.
I work with HTML, CSS, and JavaScript, and I’ve built multiple real-world projects including UI clones, interactive apps, digital tools, and responsive landing pages.
I’m open to freelance work and entry-level developer roles where I can learn, grow, and contribute.

---

## 🚀 What I Do
- 🔹 Responsive websites with **HTML, CSS, JavaScript**
- 🔹 UI Clones of popular platforms (Amazon, Netflix, etc.)
- 🔹 Landing pages for startups & local businesses
- 🔹 Website optimization & bug fixing
- 🔹 MS Office (Word, Excel, PowerPoint) tasks

---

## 🛠 Tech Stack
- **Frontend:** HTML5, CSS3, JavaScript (ES6+)
- **Tools:** Git, GitHub, VS Code, Chrome DevTools
- **Other:** MS Office Suite, Canva (basic design)

---

## 📌 Featured Projects
| Project | View Live | Source Code |
|--------|-----------|-------------|
| 🕒 Digital Clock | [View Live](https://birat-patel.github.io/Digital-Clock/) | [Source Code](https://github.com/birat-patel/Digital-Clock) |
| ➕ Calculator App | [View Live](https://birat-patel.github.io/Calculator/) | [Source Code](https://github.com/birat-patel/Calculator) |
| 🛒 Amazon UI Clone | Not Available | [Source Code](https://github.com/birat-patel/Amazon-clone) |
| 🎬 Netflix UI Clone | Not Available | [Source Code](https://github.com/birat-patel/Netflix_Clone) |
| 🌐 Personal Portfolio | [View Live](https://birat-patel.github.io/) | [Source Code](https://github.com/birat-patel/birat-patel.github.io) |

---

## 💼 Hire Me
I'm available for freelance work:  
- 🌐 Build your **responsive website**  
- 🎯 Create **UI clones & landing pages**  
- 🛠 Fix bugs & maintain your site  

📩 **How to reach me:**  
- Email: **birat802219@gmail.com**  
- WhatsApp: [Chat on WhatsApp](https://wa.me/917544957099)
- LinkedIn: [linkedin.com/in/birat-patel](https://www.linkedin.com/in/birat-patel)

---

## ✨ Fun Fact
> "I learn by building — every project you see here started with curiosity and ended with skills I can use to help you."

---

⭐ If you like my work, don’t forget to **star** my repositories!
